...
class Endomondo_Proxy
{
    ...
    */
    ...
    public function  get_workout_summary_list($max_results = 2, $before = null)
    {
        ...
        $request= $this->do_request(Endomondo_Proxy::URL_WORKOUTS, $params);
     ...
     *
     */
    public function  get_locations(Workout_Summary $workout )
    {
    ...
    private function request_auth_token($email, $password)
    {
        ...
        $request = $this->requestsEngine->get(Endomondo_Proxy::URL_AUTH, $data);
    ...
    private function do_request($url, $data = array())
    {
        ...
        $request = $this->requestsEngine->get($url, $data);

        if ($request['status_code'] != "200") {
            throw new \Exception('Bed request! StatusCode: ' . $request['status_code']);
        }
        return $request['body'];
    }
